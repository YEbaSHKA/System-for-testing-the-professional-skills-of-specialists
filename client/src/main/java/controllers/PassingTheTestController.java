...
public class PassingTheTestController implements Initializable
{
    ...
    void send_results_cclick(MouseEvent event) 
    {
        ...
        UserState.client.sendMessage("getResultOfPassTheTest");

        UserState.client.sendObject(values);

        TestEmployee test_employee = new TestEmployee();
        test_employee.setId_test(UserState.current_test_id );
        test_employee.setId_employee(UserState.current_user_id);

        UserState.client.sendObject(test_employee);
        
        if((boolean) UserState.client.readObject())
        {
            validation.AlertWindow.successPass(UserState.client.readObject().toString());
        }
        else
        {
            validation.AlertWindow.showAlertWithServerTrouble();
        }
    }
    ...
    void start_button_click(MouseEvent event) 
    {
        ...
        UserState.client.sendObject(UserState.current_test_id );
